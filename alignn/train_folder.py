...
def train_for_folder(
    ...
    '''
    import pickle
    #alignn_nmc_vfinal.pickle
    with open('alignn_nmc_eig_HSE_individual_node_v3_april23_pbe.pickle','rb') as handle:
        dataset=pickle.load(handle)
    #print(dataset[0]['nf'])
    print(dataset)
    for ik in range(len(dataset)):
        #dataset[ik]['nf']=dataset[ik]['nf'].float()
        dataset[ik]['target']=float(dataset[ik]['target'])
    print('lenght of dataset is:',len(dataset))
    if multioutput:
    ...
    '''
    print(config.atom_features)
    print('*****'*10)
    print(config)
    print('*****'*10)
    #setattr(config,atom_input_features,18)
  #  config['atom_input_features']=18
    import sys
    # sys.exit(1)
    (
        train_loader,
